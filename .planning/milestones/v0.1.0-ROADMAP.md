...
Establish project foundation with TypeScript/Vercel scaffolding, linting, and testing infrastructure.
...
**Key Decisions:**
- ESM module type for NodeNext module resolution
- Exact version pinning for reproducible builds
- Apache-2.0 license for open source
- Biome 2.x with assist.actions for organize imports
- Vitest passWithNoTests for clean CI
- Web Standard APIs (Request/Response) for portable Vercel functions

**Issues Resolved:**
- Biome 2.x schema changes (organizeImports location)
- .claude directory exclusion from lint
...
- Unused `request` parameter in api/health.ts (Biome warning - expected for health endpoints)
- Node version mismatch warning (local 23.6 vs required 24.0 - Vercel uses correct version)
