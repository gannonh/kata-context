...
must_haves:
  truths:
    ...
    - "Tests run against real PostgreSQL behavior (PGlite)"
  artifacts:
...
- TypeScript compiles without errors: `pnpm tsc --noEmit`
  ...
  <done>PGlite installed and test setup file created with migration support</done>
</task>
...
2. ContextRepository.create() returns context with auto-generated UUID and timestamps
3. ContextRepository.findById() excludes soft-deleted contexts
4. ContextRepository.softDelete() sets deletedAt timestamp
5. All tests pass against PGlite (real PostgreSQL behavior)
6. TypeScript compilation succeeds with full type inference
</success_criteria>
