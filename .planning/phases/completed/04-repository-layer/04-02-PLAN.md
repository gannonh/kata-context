...
<task type="auto">
  ...
  <action>
1. Append to `src/repositories/types.ts` after the RepositoryError class definition:

```typescript
// Add these types AFTER the RepositoryError class (around line 25):
...
Also add the NewMessage import at the top of types.ts if not already present:
```typescript
...
NOTE: Transaction wiring with FOR UPDATE is verified via tests in Task 2 (specifically "updates context counters atomically" and "assigns sequential version numbers" tests prove the transaction is working correctly).
  </action>
  <verify>
...
These tests verify the transaction with FOR UPDATE is working correctly:
- "assigns sequential version numbers" - proves version locking works
- "updates context counters atomically" - proves counter updates are atomic
- "continues version sequence from existing messages" - proves latestVersion is read correctly under lock
  </action>
  <verify>
